...
def bias_normalization(input_folder, output_file):
    '''
    Main function that prepares a dataset from the raw challenge data to an N4bias dataset
    '''
    ...
    for train_test in ['test', 'train', 'validation']:
        for folder in file_list[train_test]:
            ...
            baseFilePath = os.path.join(input_folder, folder)
            biasFilePath = os.path.join(output_file, folder)
            correct_bias(baseFilePath + "/" + folder + "_t1.nii.gz", biasFilePath + "/" + folder + "_t1.nii.gz")
            correct_bias(baseFilePath + "/" + folder + "_t1.nii.gz", biasFilePath + "/" + folder + "_t1ce.nii.gz")
            correct_bias(baseFilePath + "/" + folder + "_t1.nii.gz", biasFilePath + "/" + folder + "_t2.nii.gz")
            correct_bias(baseFilePath + "/" + folder + "_t1.nii.gz", biasFilePath + "/" + folder + "_flair.nii.gz")
...
def prepare_data(input_folder, output_file, size, input_channels, target_resolution):
    ...
    for i in os.listdir(input_folder):
        for folder in os.listdir(input_folder + i):
            print(folder)
            train_test = test_train_val_split(pid)
            pid = pid + 1
            file_list[train_test].append(i + '/' + folder)
    ...
    for train_test in ['test', 'train', 'validation']:
        for folder in file_list[train_test]:
            ...
            baseFilePath = os.path.join(input_folder, folder)
            img_c1, _, img_header = utils.load_nii(baseFilePath + "/" + "t1.nii.gz")
            img_c2, _, _ = utils.load_nii(baseFilePath + "/" + "t1ce.nii.gz")
            img_c3, _, _ = utils.load_nii(baseFilePath + "/" + "t2.nii.gz")
            img_c4, _, _ = utils.load_nii(baseFilePath + "/" + "flair.nii.gz")
    ...
    for train_test in ['train', 'test', 'validation']:
        ...
        for folder in file_list[train_test]:
            ...
            baseFilePath = os.path.join(input_folder, folder)
            segFilePath = os.path.join('/home/dell/data/Dataset/Brats19/MICCAI_BraTS_2019_Data_Training_propress/', folder)
            img_c1, _, img_header = utils.load_nii(baseFilePath + "/" + "t1.nii.gz")
            img_c2, _, _ = utils.load_nii(baseFilePath + "/" + "t1ce.nii.gz")
            img_c3, _, _ = utils.load_nii(baseFilePath + "/" + "t2.nii.gz")
            img_c4, _, _ = utils.load_nii(baseFilePath + "/" + "flair.nii.gz")
            mask_dat, _, _ = utils.load_nii(segFilePath + "/" +"seg.nii.gz")
...
def load_and_maybe_process_data(input_folder,
    ...
    data_file_name = 'N4bias_data_3D_size_%s_res_%s_fold3.hdf5' % (size_str, res_str)
...
if __name__ == '__main__':
    input_folder = "/home/dell/data/Dataset/Brats19/training_process/"
    preprocessing_folder = "/home/dell/data/Dataset/Brats19/N4bias_Training/"
    #bias_normalization(input_folder, preprocessing_folder)

    d = load_and_maybe_process_data(input_folder, preprocessing_folder, (160, 192, 160), 4, (1.0, 1.0, 1.0),
