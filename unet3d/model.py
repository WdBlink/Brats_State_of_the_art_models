...
class NoNewNet(nn.Module):
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='crg', num_groups=8,
        ...
        self.firstConv = nn.Sequential(
            nn.Conv3d(960, 480, kernel_size=1, stride=1, padding=0, bias=True),
            SynchronizedBatchNorm3d(480),
            nn.LeakyReLU()
            # nn.Conv3d(in_channels*2, in_channels, kernel_size=1, stride=1, padding=0, bias=True),
        )
...
class FeatureSelectNet(nn.Module):
    ...
    def forward(self, *input):
        ...
        # graph_ = self.firstConv(graph)
        # x_ = self.firstConv(x)
        # x = x_ - graph_
        x = input[0]
        graph = input[1]
        for i in range(self.levels):
            graph = self.encoders[i](graph)
        ...
        attention = torch.cat((x, graph), 1)
        attention = self.firstConv(attention)
...
class FeatureMinusNet(nn.Module):
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='crg', num_groups=8,
                 **kwargs):
        super(FeatureMinusNet, self).__init__()
        channels = 30
        self.levels = 5

        self.firstConv = nn.Sequential(
            nn.Conv3d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=True),
            SynchronizedBatchNorm3d(in_channels),
            nn.ReLU()
            # nn.Conv3d(in_channels*2, in_channels, kernel_size=1, stride=1, padding=0, bias=True),
        )
        self.softmax = nn.Softmax(dim=1)
...
class DistanceNoNewNet(nn.Module):
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='crg', num_groups=8,
                 **kwargs):
        super(DistanceNoNewNet, self).__init__()
        channels = 30
        self.levels = 5
        self.lastConv = nn.Conv3d(channels, 3, 1, bias=True)
...
class DMFUnit(nn.Module):
    ...
    def __init__(self, num_in, num_out, g=1, stride=1,norm=None,dilation=None):
        ...
        self.conv3x3x3_m2 = DilatedConv3DBlock(num_out, num_out, kernel_size=(3, 3, 1), stride=1, g=g,d=(1,1,1), norm=norm)
...
class MFNet(nn.Module): #
    ...
    def __init__(self, in_channels=4,n=32,channels=128,num_groups = 16,norm='bn', out_channels=3):
        super(MFNet, self).__init__()

        # Entry flow
        self.encoder_block1 = nn.Conv3d( in_channels, n, kernel_size=3, padding=1, stride=2, bias=False)# H//2
        self.encoder_block2 = nn.Sequential(
            MFunit(n, channels, g=num_groups, stride=2, norm=norm),# H//4 down
            MFunit(channels, channels, g=num_groups, stride=1, norm=norm),
            MFunit(channels, channels, g=num_groups, stride=1, norm=norm)
        )
        #
        self.encoder_block3 = nn.Sequential(
            MFunit(channels, channels*2, g=num_groups, stride=2, norm=norm), # H//8
            MFunit(channels * 2, channels * 2, g=num_groups, stride=1, norm=norm),
            MFunit(channels * 2, channels * 2, g=num_groups, stride=1, norm=norm)
        )

        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            MFunit(channels*2, channels*3, g=num_groups, stride=2, norm=norm), # H//16
            MFunit(channels*3, channels*3, g=num_groups, stride=1, norm=norm),
            MFunit(channels*3, channels*2, g=num_groups, stride=1, norm=norm),
        )

        self.upsample1 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//8
        self.decoder_block1 = MFunit(channels*2+channels*2, channels*2, g=num_groups, stride=1, norm=norm)

        self.upsample2 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//4
        self.decoder_block2 = MFunit(channels*2 + channels, channels, g=num_groups, stride=1, norm=norm)

        self.upsample3 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//2
        self.decoder_block3 = MFunit(channels + n, n, g=num_groups, stride=1, norm=norm)
        self.upsample4 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H
        self.seg = nn.Conv3d(n, out_channels, kernel_size=1, padding=0,stride=1,bias=False)
...
class DMFNet(MFNet): # sigmoid
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self,  in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='crg', num_groups=8,
                 n=32, channels=128, norm='sync_bn', **kwargs):
        super(DMFNet, self).__init__(in_channels,n,channels,num_groups, norm, out_channels)

        self.encoder_block2 = nn.Sequential(
            DMFUnit(n, channels, g=num_groups, stride=2, norm=norm,dilation=[1,2,3]),# H//4 down
            DMFUnit(channels, channels, g=num_groups, stride=1, norm=norm,dilation=[1,2,3]), # Dilated Conv 3
            DMFUnit(channels, channels, g=num_groups, stride=1, norm=norm,dilation=[1,2,3])
        )

        self.encoder_block3 = nn.Sequential(
            DMFUnit(channels, channels*2, g=num_groups, stride=2, norm=norm,dilation=[1,2,3]), # H//8
            DMFUnit(channels * 2, channels * 2, g=num_groups, stride=1, norm=norm,dilation=[1,2,3]),# Dilated Conv 3
            DMFUnit(channels * 2, channels * 2, g=num_groups, stride=1, norm=norm,dilation=[1,2,3])
        )